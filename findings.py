...
class FindingsEngine:
    ...
    def bootstrap(self, events):
        ...
        if not self.enabled:
            return None
        replayed = 0
        for event in sorted(events or [], key=lambda item: self._to_epoch(item.get("timestamp"))):
    ...
    def seed_device_history(self, history):
        ...
        for ap in wifi.get("access_points") or []:
            ...
            self.wifi_aps[bssid] = {
                ...
                "last_seen_epoch": self._to_epoch(ap.get("last_seen") or ap.get("first_seen")) or 0,
                "ssid": ap.get("ssid") or "",
                "rssi": ap.get("signal_latest"),
                "channel": (ap.get("channels") or [None])[-1] if isinstance(ap.get("channels"), list) else None,
                "source": "wifi_monitor" if "wifi_monitor" in (ap.get("sources") or []) else "wifi",
        ...
        for client in wifi.get("clients") or []:
            ...
            self.wifi_clients[mac] = {
                ...
                "last_seen_epoch": self._to_epoch(client.get("last_seen") or client.get("first_seen")) or 0,
                "ssid": "",
                "rssi": client.get("signal_latest"),
                "blank_reported": bool(client.get("blank_ssid_count")),
                "source": "wifi_monitor" if "wifi_monitor" in (client.get("sources") or []) else "wifi",
    ...
    def process(self, event, emit=True):
        ...
        if collector in ("wifi", "wifi_monitor"):
            findings.extend(self._process_wifi(collector, event_type, event, timestamp, now, emit))
        elif collector == "ble":
            findings.extend(self._process_ble(event_type, event, timestamp, now, emit))
        elif collector == "ble_identify":
            findings.extend(self._process_ble_identify(event_type, event, timestamp, emit))
        elif collector == "bt_classic":
            findings.extend(self._process_bt_classic(event_type, event, timestamp, now, emit))
        elif collector == "rtlsdr":
            findings.extend(self._process_rtlsdr(event_type, event, timestamp, emit))
    ...
    def _process_wifi(self, source, event_type, event, timestamp, now, emit):
        ...
        if event_type == "interface_mode" and data.get("warning"):
            return self._finding_list(timestamp, "warning", source, "wifi_fallback_mode", "Wi-Fi fallback mode", data.get("warning"), "wifi-fallback-mode", emit)
        return []
    ...
    def _wifi_probe_request(self, source, data, timestamp, now, emit):
        ...
        was_strong = self._is_strong(previous.get("rssi") if previous else None, self.config["strong_wifi_rssi"])
        ...
        if previous is None or not was_active or self._is_return(previous, now):
            findings.extend(self._finding_list(timestamp, "info", source, finding_type, title, detail, "wifi-client-presence:{}".format(mac), emit, self.wifi_client_attributes(mac, ssid, data)))

        if not ssid and not blank_reported:
            findings.extend(self._finding_list(timestamp, "info", source, "wifi_probe_blank_ssid", "Blank Wi-Fi probe", "Client {} sent a probe request without an SSID".format(mac), "wifi-blank-probe:{}".format(mac), emit, self.wifi_client_attributes(mac, "", data)))

        if self._is_randomized_mac(mac) and previous is None:
            findings.extend(self._finding_list(timestamp, "info", source, "wifi_randomized_mac", "Possible randomized Wi-Fi MAC", "{} has the locally administered MAC bit set".format(mac), "wifi-random-mac:{}".format(mac), emit, self.wifi_client_attributes(mac, ssid, data)))

        if self._is_strong(rssi, self.config["strong_wifi_rssi"]) and not was_strong:
            findings.extend(self._finding_list(timestamp, "warning", source, "wifi_client_strong", "Strong nearby Wi-Fi client", "{} probe RSSI is {} dBm".format(mac, rssi), "wifi-client-strong:{}".format(mac), emit, self.wifi_client_attributes(mac, ssid, data)))
    ...
    def _wifi_probe_burst(self, source, mac, ssid, timestamp, now, emit):
        ...
        return self._finding_list(
            ...
            "wifi_probe_burst",
            "Wi-Fi probe burst",
            "Client {} sent {} probe requests in {} seconds".format(mac, len(history), int(window)),
    ...
    def _wifi_ap_beacon(self, source, data, timestamp, now, emit):
        ...
        was_active = bool(previous and previous.get("active", True))
        was_strong = self._is_strong(previous.get("rssi") if previous else None, self.config["strong_wifi_ap_rssi"])
        ...
        if previous is None or not was_active or self._is_return(previous, now):
            findings.extend(self._finding_list(timestamp, "info", source, finding_type, title, detail, "wifi-ap-presence:{}".format(bssid), emit, self.wifi_ap_attributes(bssid, ssid, data)))

        if self._is_strong(rssi, self.config["strong_wifi_ap_rssi"]) and not was_strong:
            findings.extend(self._finding_list(timestamp, "warning", source, "wifi_ap_strong", "Strong nearby Wi-Fi access point", "{} RSSI is {} dBm".format(ssid or bssid, rssi), "wifi-ap-strong:{}".format(bssid), emit, self.wifi_ap_attributes(bssid, ssid, data)))

        return findings

    def _wifi_disruption(self, source, event_type, data, timestamp, emit):
        title = "Wi-Fi deauth frame observed" if event_type == "deauth_seen" else "Wi-Fi disassociation frame observed"
        ...
        return self._finding_list(
            ...
            title,
            detail,
            "{}:{}:{}".format(event_type, data.get("client_mac"), data.get("ap_mac")),
            emit,
            {"mac": data.get("client_mac") or "", "bssid": data.get("ap_mac") or ""},
        )
    ...
    def _process_ble(self, event_type, event, timestamp, now, emit):
        ...
        if event_type == "hardware_fallback":
            return self._finding_list(timestamp, "warning", "ble", "ble_fallback_mode", "BLE fallback adapter", data.get("warning") or "Using fallback BLE adapter", "ble-fallback-mode", emit)
        if event_type in ("collector_offline", "collector_retrying"):
    ...
    def _ble_device_seen(self, data, timestamp, now, emit):
        ...
        was_active = bool(previous and previous.get("active", True))
        was_strong = self._is_strong(previous.get("rssi") if previous else None, self.config["strong_ble_rssi"])
        ...
        if previous is None or not was_active or self._is_return(previous, now):
            findings.extend(self._finding_list(timestamp, "info", "ble", finding_type, title, detail, "ble-device-presence:{}".format(mac), emit, self.ble_attributes(mac, name, data)))

        if self._is_strong(rssi, self.config["strong_ble_rssi"]) and not was_strong:
            findings.extend(self._finding_list(timestamp, "warning", "ble", "ble_device_strong", "Strong nearby BLE device", "{} RSSI is {} dBm".format(name or mac, rssi), "ble-device-strong:{}".format(mac), emit, self.ble_attributes(mac, name, data)))
    ...
    def _ble_device_updated(self, data, timestamp, now, emit):
        ...
        if old_rssi is None or new_rssi is None or abs(new_rssi - old_rssi) < float(self.config["rssi_change_db"]):
            return []
    ...
    def _ble_device_lost(self, data, timestamp, emit):
        ...
        return self._finding_list(timestamp, "info", "ble", "ble_device_lost", "BLE device disappeared", "{} has not been seen recently".format(label), "ble-device-lost:{}".format(mac), emit, self.ble_attributes(mac, known.get("name") or "", known))
    ...
    def _process_ble_identify(self, event_type, event, timestamp, emit):
        ...
        if event_type == "identify_result":
            ...
            ).strip() or "Device Information Service fields were read"
            return self._finding_list(
                timestamp,
                "info",
                "ble_identify",
    ...
    def _process_bt_classic(self, event_type, event, timestamp, now, emit):
        ...
        if event_type in ("classic_device_seen", "classic_device_updated"):
            ...
            if previous is None or not was_active or self._is_return(previous, now):
                title = "New classic Bluetooth device" if previous is None else "Classic Bluetooth device returned"
                detail = "{} ({})".format(name, mac) if name else mac
                return self._finding_list(timestamp, "info", "bt_classic", "bt_classic_device_seen", title, detail, "bt-classic-presence:{}".format(mac), emit, self.bt_classic_attributes(mac, data))
        if event_type == "classic_device_lost":
            ...
            label = known.get("name") or mac
            return self._finding_list(timestamp, "info", "bt_classic", "bt_classic_device_lost", "Classic Bluetooth device disappeared", "{} has not been seen recently".format(label), "bt-classic-lost:{}".format(mac), emit, self.bt_classic_attributes(mac, known))
        if event_type == "hardware_fallback":
            return self._finding_list(timestamp, "warning", "bt_classic", "bt_classic_fallback_mode", "Bluetooth classic fallback adapter", data.get("warning") or "Using fallback Bluetooth adapter", "bt-classic-fallback-mode", emit)
        if event_type in ("collector_offline", "collector_retrying"):
            return self._collector_warning("bt_classic", event_type, data, timestamp, emit)
        return []
    ...
    def _process_rtlsdr(self, event_type, event, timestamp, emit):
        ...
        if event_type == "signal_detected":
            ...
            return self._finding_list(timestamp, "warning", "rtlsdr", "rtlsdr_signal_detected", "RTL-SDR signal detected", "{} MHz is {} dB above baseline".format(frequency, data.get("above_floor_db")), "rtlsdr-signal:{}".format(frequency), emit)
        if event_type == "signal_lost":
            frequency = data.get("frequency_mhz")
            self.rtlsdr_signals.pop(frequency, None)
            return self._finding_list(timestamp, "info", "rtlsdr", "rtlsdr_signal_lost", "RTL-SDR signal lost", "{} MHz returned below threshold".format(frequency), "rtlsdr-signal-lost:{}".format(frequency), emit)
        if event_type == "collector_offline":
            return self._collector_warning("rtlsdr", event_type, data, timestamp, emit)
        return []
    ...
    def _process_system(self, event_type, event, timestamp, emit):
        ...
        if event_type not in ("collector_loaded", "collector_started", "collector_stopped", "collector_already_running"):
            return []
        key = data.get("key")
        ...
        if state == previous:
            return []
        if state == "STOPPED":
            return self._collector_state_finding(timestamp, "info", key, data, "collector_stopped", "stopped", emit)
        if state == "OFFLINE":
            return self._collector_state_finding(timestamp, "warning", key, data, "collector_offline", "offline", emit)
        if state == "RETRYING":
            return self._collector_state_finding(timestamp, "warning", key, data, "collector_retrying", "retrying", emit)
        return []

    def _collector_state_finding(self, timestamp, severity, key, data, finding_type, state_text, emit):
    ...
    def _process_system_status(self, data, timestamp, emit):
        ...
        if wifi.get("iw") is False and wifi.get("iwlist") is False:
            findings.extend(self._finding_list(timestamp, "warning", "wifi", "missing_executable", "iw/iwlist missing", "Wi-Fi scan needs iw or iwlist for managed AP scans", "wifi:missing-executable:iw-iwlist", emit))
        if wifi_monitor.get("iw") is False:
            findings.extend(self._finding_list(timestamp, "warning", "wifi_monitor", "missing_executable", "iw missing", "Wi-Fi monitor channel detection executable was not located", "wifi-monitor:missing-executable:iw", emit))
        if wifi_monitor.get("scapy") is False:
            findings.extend(self._finding_list(timestamp, "warning", "wifi_monitor", "missing_python_package", "scapy missing", "Wi-Fi monitor packet capture package is not installed", "wifi-monitor:missing-package:scapy", emit))
        if ble.get("bleak") is False:
            findings.extend(self._finding_list(timestamp, "warning", "ble", "missing_python_package", "bleak missing", "BLE scanning package is not installed", "ble:missing-package:bleak", emit))
        if bt_classic.get("hcitool") is False and bt_classic.get("bluetoothctl") is False:
            findings.extend(self._finding_list(timestamp, "warning", "bt_classic", "missing_executable", "Bluetooth classic scanner missing", "Classic Bluetooth scan needs hcitool or bluetoothctl", "bt-classic:missing-executable", emit))
        if rtlsdr.get("rtl_power") is False:
            findings.extend(self._finding_list(timestamp, "warning", "rtlsdr", "missing_executable", "rtl_power missing", "RTL-SDR spectrum executable was not located", "rtlsdr:missing-executable:rtl_power", emit))
        if rtlsdr.get("rtl_test") is False:
            findings.extend(self._finding_list(timestamp, "warning", "rtlsdr", "missing_executable", "rtl_test missing", "RTL-SDR device validation executable was not located", "rtlsdr:missing-executable:rtl_test", emit))
    ...
    def _rtlsdr_persistent_signals(self, timestamp, emit):
        ...
        for frequency, data in self.rtlsdr_signals.items():
            ...
            findings.extend(self._finding_list(
                timestamp,
                "warning",
                "rtlsdr",
                "rtlsdr_signal_persistent",
                "RTL-SDR signal persisted",
                "{} MHz has stayed above baseline for at least {} seconds".format(frequency, int(threshold)),
    ...
    def _collector_warning(self, source, event_type, data, timestamp, emit):
        reason = data.get("reason") or data.get("warning") or data.get("error") or event_type
        title = "{} {}".format(source.upper(), "offline" if event_type == "collector_offline" else "retrying")
        return self._finding_list(timestamp, "warning", source, event_type, title, reason, "{}:{}".format(source, event_type), emit)
    ...
    def _expire_presence(self, timestamp, now):
        ...
        for mac, data in self.wifi_clients.items():
            if data.get("active", True) and now - data.get("last_seen_epoch", now) > lost_after:
                data["active"] = False
                source = data.get("source") or "wifi"
                findings.extend(self._finding_list(timestamp, "info", source, "wifi_client_lost", "Wi-Fi client disappeared", "{} has not sent probes recently".format(mac), "wifi-client-lost:{}".format(mac), True, self.wifi_client_attributes(mac, data.get("ssid") or "", data)))
        for bssid, data in self.wifi_aps.items():
            if data.get("active", True) and now - data.get("last_seen_epoch", now) > lost_after:
                data["active"] = False
                source = data.get("source") or "wifi"
                findings.extend(self._finding_list(timestamp, "info", source, "wifi_ap_lost", "Wi-Fi access point disappeared", "{} has not beaconed recently".format(data.get("ssid") or bssid), "wifi-ap-lost:{}".format(bssid), True, self.wifi_ap_attributes(bssid, data.get("ssid") or "", data)))
        return findings

    def _finding_list(self, timestamp, severity, source, finding_type, title, detail, key, emit, attributes=None):
        if not emit and finding_type != "findings_history_loaded":
            return []
        finding = self._finding(timestamp, severity, source, finding_type, title, detail, key, attributes=attributes)
        if finding and (emit or finding_type == "findings_history_loaded"):
            return [finding]
        return []

    def _finding(self, timestamp, severity, source, finding_type, title, detail, key, force=False, attributes=None):
        now = self._to_epoch(timestamp)
        last = self._last_emitted.get(key)
        cooldown = float(self.config.get("cooldown_sec", 120))
        if not force and last and last.get("severity") == severity and now - last.get("epoch", 0) < cooldown:
            return None
    ...
    def _is_return(self, previous, now):
        return now - previous.get("last_seen_epoch", now) >= float(self.config["return_after_sec"])
