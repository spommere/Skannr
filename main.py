...
from collectors.metadata import browser_source_groups, browser_subtabs, collector_definitions, collector_keys
...
from log_utils import count_jsonl_files, current_jsonl_checkpoint, event_in_window, has_jsonl_checkpoint, read_incremental_jsonl_events, read_jsonl_events, resolve_window_days as resolve_log_window_days, utc_epoch, view_window_options, window_metadata
...
def events():
    ...
    enqueue_sse(client, "collector_status", [collector.status() for collector in runtime["collectors"]])
    enqueue_sse(client, "system_status", system_status())
    enqueue_sse(client, "findings_snapshot", runtime["findings"].snapshot())
    enqueue_sse(client, "skannr_event", {
        "collector": "system",
        "timestamp": utc_now(),
...
def ble_identify():
    ...
    if not loop or not collector:
        return {"ok": False, "error": "BLE Identify collector is not available"}, 503
    if not mac:
...
def view_metadata():
    ...
    return {
        ...
        "collectors": {
            "rtlsdr": {
                "scan_start_mhz": config.get("collectors", {}).get("rtlsdr", {}).get("scan_start_mhz"),
                "scan_end_mhz": config.get("collectors", {}).get("rtlsdr", {}).get("scan_end_mhz"),
                "step_khz": config.get("collectors", {}).get("rtlsdr", {}).get("step_khz"),
                "gain": config.get("collectors", {}).get("rtlsdr", {}).get("gain"),
                "threshold_db": config.get("collectors", {}).get("rtlsdr", {}).get("threshold_db"),
            },
        },
    }
...
def device_history():
    ...
    return cached_derived_view("device_history", load_cached_device_history, window_days)
...
def device_history_refresh():
    ...
    return build_derived_views(requested_window_days(), force=True)["device_history"]
...
def findings_history():
    ...
    window_days = requested_window_days()
    return cached_derived_view("findings_history", load_cached_findings_history, window_days)
...
def history_analysis():
    ...
    window_days = requested_window_days()
    return cached_derived_view("history_analysis", load_cached_history_analysis, window_days)


@app.route("/history_analysis/refresh", methods=["POST"])
def history_analysis_refresh():
    """Compatibility route: refresh the full derived-data bundle."""
    return build_derived_views(requested_window_days(), force=True)["history_analysis"]
...
def on_connect():
    ...
    socketio.emit("collector_status", [collector.status() for collector in runtime["collectors"]])
    socketio.emit("system_status", system_status())
    socketio.emit("findings_snapshot", runtime["findings"].snapshot())
    socketio.emit("skannr_event", {
        "collector": "system",
        "timestamp": utc_now(),
        "type": "browser_connected",
        "severity": "info",
...
async def consume_events(bus):
    ...
    while True:
        ...
        high_rate_state_event = (
            ...
            event.get("collector") == "wifi_monitor" and event.get("type") == "monitor_channel_changed"
        )
        ...
        broadcast("collector_status", [collector.status() for collector in runtime["collectors"]])
...
async def start_collectors(config, bus):
    ...
    await bus.publish({
        ...
        "type": "app_started",
        "severity": "info",
        "data": {"message": "Skannr event bus is online", "timestamp": utc_now()},
    })
...
def cached_derived_view(key, loader, window_days):
    ...
    if runtime.get(key) is None or not summary_matches_window(runtime.get(key), window_days):
...
def read_findings_history(window_days):
    ...
    findings.sort(key=lambda item: utc_epoch(item.get("timestamp")) or 0, reverse=True)
    ...
    return {
        ...
        "counts": {
            ...
            "error": sum(1 for item in findings if item.get("severity") in ("error", "alert")),
        },
    }
...
def refresh_findings_history(window_days):
    ...
    if previous and has_jsonl_checkpoint(previous):
        ...
        summary["checkpoint"] = current_jsonl_checkpoint(configured_log_dir(), ("findings",))
...
def update_findings_summary(previous):
    ...
    records_read = 0
    for event in read_incremental_jsonl_events(configured_log_dir(), "findings", checkpoint):
        records_read += 1
        if event.get("type") == "finding" and event.get("data"):
            findings.append(normalize_finding_source(event["data"]))
    findings.sort(key=lambda item: utc_epoch(item.get("timestamp")) or 0, reverse=True)
...
def display_findings_summary(summary, window_days):
    ...
    if window_days is not None:
        findings = [
            ...
            if event_in_window({"timestamp": item.get("timestamp"), "data": item}, window_days)
        ]
    output["findings"] = findings
    output["window"] = view_window_metadata(window_days)
    output["materialized_window"] = (summary or {}).get("window") or view_window_metadata(None)
...
def build_derived_views(window_days="default", force=False):
    ...
    if force:
        ...
        cached_derived_view("findings_history", load_cached_findings_history, window_days)
        cached_derived_view("device_history", load_cached_device_history, window_days)
        cached_derived_view("history_analysis", load_cached_history_analysis, window_days)
    ...
    return {
        ...
        "window": view_window_metadata(window_days),
        "findings": add_refresh_metadata(runtime["findings_history"], generated_at, generated_at_epoch),
        "device_history": add_refresh_metadata(runtime["device_history"], generated_at, generated_at_epoch),
        "history_analysis": add_refresh_metadata(runtime["history_analysis"], generated_at, generated_at_epoch),
        "reports": add_refresh_metadata(runtime["reports"], generated_at, generated_at_epoch),
    }
...
def normalize_finding_source(finding):
    ...
    if finding.get("source") != "system":
        return finding
    if finding.get("type") not in ("collector_offline", "collector_retrying", "collector_stopped"):
        return finding
    known = set(collector_keys(runtime.get("config") or {}, include_system=False))
...
def load_cached_device_history(window_days):
    ...
    if isinstance(summary, dict):
        ...
        output = DeviceHistoryBuilder(configured_log_dir(), state_path=device_history_path(), window_days=window_days).display_summary(summary, window_days)
        output["cached"] = True
        return output
...
def load_cached_history_analysis(window_days):
    ...
    if isinstance(analysis, dict):
        ...
        analysis.setdefault("counts", count_observations(analysis.get("observations") or []))
...
def refresh_history_analysis(window_days="default"):
    ...
    if history is None:
        return {"generated_at": utc_now(), "observations": [], "counts": {"total": 0, "warning": 0, "info": 0}}
    config = runtime.get("config") or {}
    ...
    state_path = history.get("state_path") or os.path.join("logs", "device_history", "device_history.json")
...
async def shutdown_runtime():
    ...
    for collector in runtime["collectors"]:
        try:
            ...
            logging.exception("failed to stop collector %s: %s", collector.config_key, exc)
    pending = [task for task in runtime["tasks"] if task is not current and not task.done()]
    for task in pending:
        task.cancel()
    if pending:
