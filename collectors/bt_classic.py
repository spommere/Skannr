...
class BluetoothClassicCollector(BaseCollector):
    ...
    def detect(self):
        ...
        if fallback_ok:
            ...
            self.warning = "Using fallback Bluetooth adapter {}; primary validation failed: {}".format(fallback, primary_detail)
            return True
    ...
    async def start(self):
        ...
        await self.emit("scanner_started", {"adapter": self.active_hardware, "tier": self.state, "mode": "classic"})
        ...
        while self._running:
            ...
            try:
                await self.emit("classic_scan_started", {"adapter": self.active_hardware})
            ...
            await self.emit("classic_scan_completed", {
            ...
            for device in devices:
                ...
                event_type = "classic_device_seen" if previous is None else "classic_device_updated"
                await self.emit(event_type, device)

            lost = [mac for mac, item in seen.items() if now - item["last_seen"] > timeout]
            for mac in lost:
                await self.emit("classic_device_lost", {"mac": mac, "transport": "classic"})
                del seen[mac]
    ...
    def scan_once(self):
        ...
        if shutil.which("hcitool"):
            ...
            return self.parse_hcitool_scan(self.command_output(command, timeout=float(self.config.get("scan_timeout_sec", 20))))
        return self.parse_bluetoothctl_devices(self.command_output(["bluetoothctl", "devices"], timeout=10))
    ...
    def command_output(self, command, timeout=10):
        ...
        if result.returncode != 0:
            raise RuntimeError(output.strip() or "{} exited {}".format(" ".join(command), result.returncode))
        return output
