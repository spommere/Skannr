...
class DeviceHistoryBuilder:
    ...
    def apply_wifi_event(self, event, wifi_aps, wifi_clients):
        ...
        if event_type == "ap_beacon":
            ...
            self.extend_wifi_ap_session(item, timestamp, data)
            item["observations"] += 1
    ...
    def apply_ble_event(self, event, ble_devices):
        ...
        if event_type == "device_seen":
            ...
            item["lost_count"] += 1
            self.close_ble_session(item, timestamp, data)
    ...
    def records_in_window(self, records, window_days):
        ...
        for record in records:
            event = {"timestamp": record.get("last_seen"), "data": record}
    ...
    def extend_wifi_ap_session(self, item, timestamp, data):
        ...
        if active:
            gap = self.duration_seconds(active.get("end"), timestamp)
    ...
    def duration_seconds(self, first_seen, last_seen):
        ...
        first = self.time_key(first_seen)
        last = self.time_key(last_seen)
